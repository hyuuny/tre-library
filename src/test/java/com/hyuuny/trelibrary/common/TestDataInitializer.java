...
public class TestDataInitializer {
    ...
    CommandLineRunner initData(
            ...
            PopularSearchTermRepository popularSearchTermRepository
    ) {
        return args -> {
            if (bookRepository.count() == 0) {
                initBooksData(bookRepository);
            }

            if (popularSearchTermRepository.count() == 0) {
                initPopularSearchTermsData(popularSearchTermRepository);
            }
        };
    }

    private void initPopularSearchTermsData(PopularSearchTermRepository popularSearchTermRepository) {
        List<PopularSearchTerm> popularSearchTerms = List.of(
        ...
        popularSearchTermRepository.saveAll(popularSearchTerms);
    }

    private void initBooksData(BookRepository bookRepository) {
